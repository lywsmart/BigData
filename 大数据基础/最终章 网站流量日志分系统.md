...
​	确保Hadoop集群处于启动状态下运行[HadoopDataReport.jar]()。
...
```hive
create database if not exists weblog_dw;
```

<img src="./最终章 网站流量日志分系统.assets/image-20231218211222377.png" alt="image-20231218211222377" />
...
​	在数据库weblog_dw创建[业务表weblog_pv_da]()，命令如下。
...
​	在数据库weblog_dw创建[业务表weblog_avguv_da]()，命令如下。
